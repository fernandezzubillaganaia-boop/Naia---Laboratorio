...
-- 8. Actualiza el correo electrónico de un cliente específico (ejemplo: cliente con id 1).
UPDATE Clientes
SET email_cliente = 'nuevo.email@email.com'
WHERE id_cliente = 1;

-- 9. Elimina un cliente por su ID (ejemplo: cliente con id 5).
...
-- 22. Actualiza la talla de una prenda específica por su ID (ejemplo: id_prenda = 1).
UPDATE Prendas
SET talla = 'L'
WHERE id_prenda = 1;
...
-- 25. Elimina un empleado por su ID (ejemplo: id_empleado = 3).
...
SELECT COUNT(*) AS compras_septiembre_2023
FROM Compras
WHERE fecha_compra BETWEEN '2023-09-01' AND '2023-09-30';

-- 30. Actualiza la dirección de una tienda por su ID (ejemplo: id_tienda = 2).
UPDATE Tiendas
...
SELECT color, COUNT(*) AS cantidad_prendas
FROM Prendas
GROUP BY color;
...
('Zara Sol', 'Puerta del Sol, 15', 'Madrid', 'España');
